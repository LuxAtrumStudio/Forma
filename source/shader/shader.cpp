...
void forma::shader::Shader::VertexShader(std::string shader) {
  if (FileExists(shader)) {
    ...
    estl::logger::Warning("Could not open vertex shader at \"%s\"", shader.c_str());
  }
}
...
void forma::shader::Shader::FragmentShader(std::string shader) {
  if (FileExists(shader)) {
    ...
    estl::logger::Warning("Could not open fragment shader at \"%s\"", shader.c_str());
  }
}
...
void forma::shader::Shader::CompileShader() {
  ...
  if (status == 0) {
    ...
    estl::logger::Error("%s", info_str.c_str());
    glDeleteProgram(*id_);
    id_ = NULL;
  }
...
void forma::shader::Shader::Use() {
  if (id_ != NULL) {
    ...
    estl::logger::Warning("Shader program not defined");
  }
}
...
unsigned int forma::shader::Shader::LoadShader(std::string name,
  ...
  if (file.is_open() == true) {
    ...
    if (status == 0) {
      ...
      estl::logger::Error("%s", info_string.c_str());
      glDeleteShader(shader_id);
      shader_id = 0;
    }
  } else {
    estl::logger::Error("%s shader not defined", shader_name.c_str());
  }
  return shader_id;
}
