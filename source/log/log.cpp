...
void forma::log::Log(unsigned int type, std::string msg, std::string func,
                     ...) {
  std::string str;
...
int forma::log::GetLogSize() { return global_logs.size(); }
...
std::pair<unsigned int, std::string> forma::log::FGetLog(unsigned int type) {
...
std::string forma::log::IGetLog(int index) {
  std::string entry;
...
std::pair<unsigned int, std::string> forma::log::IFGetLog(int index) {
  std::pair<unsigned int, std::string> entry;
  if (index >= 0 && (size_t)index < global_logs.size()) {
...
std::vector<std::string> forma::log::VGetLog(int start, int end) {
  std::vector<std::string> entries;
...
std::vector<std::pair<unsigned int, std::string>> forma::log::VFGetLog(
    int start, int end) {
...
void forma::log::SetLogHandle(
    void (*handle)(std::pair<unsigned int, std::string>)) {
  log_handle_full = handle;
}

void forma::log::SetLogHandle(void (*handle)(std::string)) {
  log_handle = handle;
}

void forma::log::SetLogOption(unsigned int option, int setting) {
...
void forma::log::SaveLog(std::string file) {
